...
3. Compare every event in the JSON against the official page. Look specifically for:
   - changed registration / submission / notification dates
...
- Do not add new conferences or new event kinds. Tracked kinds are `abstract`, `registration`, `submission`, `notification` and `conference`; camera-ready / final-paper deadlines are intentionally excluded, so never add them back even if the CFP lists them.
